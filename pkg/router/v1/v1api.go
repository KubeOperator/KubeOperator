...
func V1(parent iris.Party) {
	...
	mvc.New(projectScope.Party("/project/{project}/cluster/{cluster}/members")).HandleError(ErrorHandler).Handle(controller.NewClusterMemberController())
	mvc.New(projectScope.Party("/project/{project}/cluster/{cluster}/resources")).HandleError(ErrorHandler).Handle(controller.NewClusterResourceController())
	WhiteScope = v1.Party("/")
