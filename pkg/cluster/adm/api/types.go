...
type InitConfiguration struct {
	...
	// When InitConfiguration is marshalled to bytes in the external version.go, this information IS NOT preserved (which can be seen from
...
type ClusterConfiguration struct {
	...
	// KubernetesVersion is the target version.go of the control plane.
	...
	// If empty, `k8s.gcr.io` will be used by default; in case of kubernetes version.go is a CI build (kubernetes version.go starts with `ci/` or `ci-cross/`)
...
type ImageMeta struct {
	...
	// In case this value is set, kubeadm does not change automatically the version.go of the above components during upgrades.
	ImageTag string
...
// Kubernetes version.go from CI build area.
