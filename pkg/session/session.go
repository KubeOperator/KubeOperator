...
func (mgr *SessionMgr) StartSession(w http.ResponseWriter, r *http.Request) string {
	...
	cookie := http.Cookie{Name: mgr.mCookieName, Value: newSessionID, Path: "/", SameSite: 2, HttpOnly: true, MaxAge: 0, Secure: true}
...
func (mgr *SessionMgr) EndSession(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || cookie.Value == "" {
		...
		expiration := time.Now()
		cookie := http.Cookie{Name: mgr.mCookieName, Path: "/", SameSite: 2, HttpOnly: true, Expires: expiration, MaxAge: -1, Secure: true}
		http.SetCookie(w, &cookie)
	}
}
