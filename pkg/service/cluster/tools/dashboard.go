...
func NewDashboard(cluster dto.ClusterWithEndpoint, tool *model.ClusterTool) (*Dashboard, error) {
	p := &Dashboard{
		Tool: tool,
	}
...
func (c Dashboard) installChart() error {
	...
	_, err := c.HelmClient.Install(c.Tool.Name, constant.DashboardChartName, valueMap)
	if err != nil {
		return nil
	}
	return nil
...
func (c Dashboard) createRoute() error {
	...
	for _, svc := range services.Items {
		if strings.Contains(svc.Name, "kubernetes-dashboard") {
