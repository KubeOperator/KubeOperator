...
func (c *Cluster) Export(ctx context.Context) ([]byte, error) {
	...
	for _, ns := range namespaces {
		namespace, err := c.client.CoreV1().Namespaces().Get(context.TODO(),ns, meta_v1.GetOptions{})
		if err != nil {
			return nil, err
		}
...
func (c *Cluster) getAllowedAndExistingNamespaces(ctx context.Context) ([]string, error) {
	if len(c.allowedNamespaces) > 0 {
		...
		for name, _ := range c.allowedNamespaces {
			...
			ns, err := c.client.CoreV1().Namespaces().Get(context.TODO(),name, meta_v1.GetOptions{})
			switch {
			case err == nil:
